...
@EnableJpaRepositories(entityManagerFactoryRef = PrimaryPersistenceContext.EM_FACTORY, transactionManagerRef = PrimaryPersistenceContext.TX_MANAGER, basePackages = {"com.example.persistence"}, repositoryBaseClass = AbstractRepositoryImpl.class)
public class PrimaryPersistenceContext {
	...
	public LocalContainerEntityManagerFactoryBean primaryEntityManagerFactory(EntityManagerFactoryBuilder builder, @Qualifier(DATASOURCE) DataSource primaryDataSource) {
		return builder.dataSource(primaryDataSource).packages("com.example.persistence.entity").persistenceUnit("primaryPSTUnit").build();
	}
